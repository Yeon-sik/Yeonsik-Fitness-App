...
public final class CardioSummaryScreen extends BaseScreen {
    ...
    public void render() {
        ...
        boolean cycling = "cycling".equals(snapshot.getActivityId());
        ...
        section("데이터 범위");
        emptyState(
                "GPS 위치 " + snapshot.getAcceptedPointCount()
                        + "개를 반영했고 거리·시간"
                        + (CardioMetrics.hasAverageHeartRate(snapshot.getAverageHeartRateBpm())
                        ? "·평균 심박수" : "")
                        + " 요약을 운동 기록으로 저장했습니다.",
