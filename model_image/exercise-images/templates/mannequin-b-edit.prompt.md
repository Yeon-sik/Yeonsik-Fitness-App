...
- compiled invisible grip targets for B (pixel coordinates): `{{invisibleGripTargetsBJson}}`

Preserve mannequin identity, body proportions, head size, camera, canvas, linework, muscle colors, and all locked joints. Do not draw or recreate canonical equipment. Use only the compiled invisible grip target coordinates above for the hand pose; the same canonical equipment identity used for A will be composited later.
