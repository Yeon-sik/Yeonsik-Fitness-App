...
| 식단 입력값의 출처와 조리상태가 불명확하다 | 공식 원재료성 식품 데이터에서 선별한 55개 행을 로컬 SQLite 카탈로그로 시드하고 100g·생것/구이 상태를 행별로 표시한다 | `VerifiedFoodCatalogSeed.java`, `verified_food_catalog_v3.json` |
...
| 공식 식품 카탈로그 | 닭·소·돼지·곡물·채소와 생선 7행(연어·참치·고등어 생것/구이, 민어 구이), 총 55행 | 원본 재생성 명령과 Android 시드 테스트 소스 |
...
| 검증 항목 | 상태 | 근거와 경계 |
| --- | --- | --- |
| 공식 카탈로그 생성 | 통과 | `scripts/generate-verified-food-catalog.ps1` 실행 결과 55행(생것 51·구이 4) |
...
- **식품 범위**: 현재는 선별한 공식 55행이다. 추가 품목은 동일한 원본 코드·공식명·조리상태·100g 기준·결측 정책을 갖춰야 한다.
